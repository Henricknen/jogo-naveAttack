...
void setup() {
  ...
  venergia = 100;
  vtela=0;
  lcd.createChar(1, nave);		// criando caracters dos elementos (nave, tiro, asteroide etc)
...
void loop(){
  if(game){ 
    ...
    if(venergia <= 0){
      ...
      vtela = 2;		// está condição indica que jogador perdeu
    }
    painel(13);		// redesenhando 'painel' após redesenha a tela
    if(digitalRead(btcima) == 1){
      pynave = 0;		// se 'btcima' for precionado a nav irá para linha de cima
    }
    ...
    if(((pxtiro == pxaste+1) && (pytiro == pyaste))||((pxtiro == pxaste) && (pytiro == pyaste))){
      vtiro=false;
      pxtiro = -1;		// posição original do tiro
      ...
      if(pontos >= maxpontos){
        game = 0;
        vtela = 1;		// 'vtela = 1' é uma condição de vitoria
      }
    }
